...
EQEditor::EQEditor (EQProcessor& proc)
      ...
      schematic (proc)
{
    ...
    setupDial (lfBoostDial, "LO BOOST", "lfboost");
    setupDial (lfCutDial,   "LO ATTEN", "lfcut");
    setupDial (hfFreqDial,  "HF FREQ",  "hffreq");
    setupDial (hfBoostDial, "HI BOOST", "hfboost");
    setupDial (hfCutDial,   "HI CUT",   "hfcut");

    // Commutateur LF : 3 crans (30 / 60 / 100 Hz), dessine par le LookAndFeel.
    {
        auto& s = lfSwitch.slider;
        ...
        auto& l = lfSwitch.name;
        l.setText ("LF", juce::dontSendNotification);
        l.setJustificationType (juce::Justification::centred);
        l.setFont (fonts::lettering (11.5f));
        l.setColour (juce::Label::textColourId, palette::ink);
        addAndMakeVisible (l);

        lfSwitch.attachment = std::make_unique<SAttach> (processor.getAPVTS(), "lf", s);
    }

    startTimerHz (30);   // pilote la courbe composite de FIG. 1
}
...
void EQEditor::mouseUp (const juce::MouseEvent& e)
{
...
void EQEditor::setupDial (Dial& d, const juce::String& labelText, const juce::String& paramID)
{
    auto& s = d.slider;
    ...
    // Le format des valeurs ("+4.0 dB", "-2.5 dB", "8.0 kHz") vient des parametres.
...
void EQEditor::drawSheetFrame (juce::Graphics& g)
{
...
void EQEditor::drawHeader (juce::Graphics& g)
{
    ...
    g.setColour (palette::ink);
    g.drawText ("EQ MXA", header.withWidth (160.0f), juce::Justification::centredLeft);

    g.setFont (fonts::lettering (10.5f));
    g.setColour (palette::inkMid);
    g.drawText ("PASSIVE-STYLE PROGRAM EQUALIZER",
                header.withX (header.getX() + 168.0f).withWidth (250.0f),
    ...
    g.drawText ("DWG NO. MXA-EQ-01", row2.withTrimmedLeft (8.0f).withTrimmedRight (86.0f),
                juce::Justification::centredLeft);
...
void EQEditor::resized()
{
    ...
    Dial* dials[] = { &lfSwitch, &lfBoostDial, &lfCutDial, &hfFreqDial, &hfBoostDial, &hfCutDial };
